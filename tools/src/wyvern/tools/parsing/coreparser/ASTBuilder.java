...
interface ASTBuilder<AST, Type> {
    ...
    AST moduleDecl(String name, List<AST> preImports, List<GenericParameter> generics,
                   List args, List<AST> postImports, AST ast, Type type, FileLocation loc, boolean isResource, boolean isAnnotated, String effects);
    ...
    AST tryStatement(Type withType, Type returnType, Type tryType, List exp, AST fields, String tryObj,
                     String with, AST breakExpr, FileLocation loc);
    AST breakExpr(AST returnExpr, FileLocation loc);
